...
df_train["arg1"] = df_train["input"].astype(str).str.replace('"', '').astype(int)
df_test["arg1"] = df_test["input"].astype(str).str.replace('"', '').astype(int)
...
def feature_engineering(df):
    ...
    df["log(arg1*16)"] = np.log1p(df["arg1"] * 16)
    df["arg1^2"] = np.log1p((df["arg1"]/16) ** 2)
    df["log(arg1)"] = np.log1p((df["arg1"]/16))
    df["min(arg1 16)"] = df["arg1"].clip(upper=16)  # Returns min(arg1, 16)
    return df
...
if os.path.exists(model_filename) and want_to_use_trained_model == 1:
    ...
    nn_model = MLPRegressor(
        ...
        activation='relu',  # Changed from ReLU to tanh
        solver='adam',
...
# average_difference = comparison_df["Absolute Difference"].mean()
...
# ===== Group mapping =====
sdc_grouping_info = [[0.0, 17], [17, 19], [19, 21], [21, 23], [23, 25], [25, 27], [27, 29], [29, 31], [31, 33], [33, 35], [35, 37], [37, 39], [39, 41], [41, 43], [43, 100.0]]
...
original_inputs = df_test_raw["input"].astype(str).str.replace('"', '')
...
for input_str, predicted_sdc in result_array:
    ...
    os.system(f"cp -r ../../b-Offline-PreProtected-Binary-Generation/e-Backprop/protected-binary-for-{pred_rep.replace(' ', '-')} protected-binary-for-{input_str.replace(' ', '-')}")
