...
df_train[["arg1", "arg2"]] = df_train["input"].str.replace('"', '').str.split(" ", expand=True).astype(int)
df_test[["arg1", "arg2"]] = df_test["input"].str.replace('"', '').str.split(" ", expand=True).astype(int)
...
def feature_engineering(df):
    ...
    df["arg1/arg2"] = df["arg1"] / df["arg2"]
    df["arg1-arg2"] = df["arg1"] - df["arg2"]
    df["arg1+arg2"] = df["arg1"] + df["arg2"]
    df["arg1*arg2"] = df["arg1"] * df["arg2"]
    df["log(arg1+arg2)"] = np.log1p(df["arg1"] + df["arg2"])
    df["log(arg1-arg2)"] = np.log1p(df["arg1"]) - np.log1p(df["arg2"])  
    df["log(arg1*arg2)"] = np.log1p(df["arg1"] * df["arg2"])  
    df["log(arg1/arg2)"] = np.log1p(df["arg1"] / df["arg2"])
    return df
...
# ===== Group mapping =====
sdc_grouping_info = [[0.0 ,18.5], [18.5, 22.0], [22.0, 25.5], [25.5, 29.0], [29.0, 32.5], [32.5, 36.0], [36.0, 39.5], [39.5, 43.0], [43.0, 46.5], [46.5, 100.0]]
...
for input_str, predicted_sdc in result_array:
    ...
    os.system(f"cp -r ../../b-Offline-PreProtected-Binary-Generation/a-Pathfinder/protected-binary-for-{pred_rep.replace(' ', '-')} protected-binary-for-{input_str.replace(' ', '-')}")
