...
df_train[["arg1", "arg2", "arg3"]] = df_train["input"].str.replace('"', '').str.split(" ", expand=True)
df_test[["arg1", "arg2", "arg3"]] = df_test["input"].str.replace('"', '').str.split(" ", expand=True)
df_train[["arg3"]] = df_train[["arg3"]].astype(int)
df_test[["arg3"]]  = df_test[["arg3"]].astype(int)

df_train.drop(columns=["input", "arg1", "arg2"], inplace=True)
df_test.drop(columns=["input", "arg1", "arg2"], inplace=True)
...
def feature_engineering(df):
    ...
    df["log(arg3*arg3)"] = np.log1p(df["arg3"] * df["arg3"])
    df["min(arg3 10)"] = df["arg3"].clip(upper=10)
    return df
...
# ===== Group mapping =====
sdc_grouping_info = [[0.0, 43.0], [43.0, 45.0],[45.0, 47.0],[47.0, 49.0],[49.0, 100.0]]
...
for input_str, predicted_sdc in result_array:
    ...
    os.system(f"cp -r ../../b-Offline-PreProtected-Binary-Generation/d-LU/protected-binary-for-{pred_rep.replace(' ', '-')} protected-binary-for-{input_str.replace(' ', '-')}")
