...
# Extracting arg1, arg2 from "input" column
df_train[["arg1", "arg2", "arg3"]] = df_train["input"].str.replace('"', '').str.split(" ", expand=True).astype(int)
df_test[["arg1", "arg2", "arg3"]] = df_test["input"].str.replace('"', '').str.split(" ", expand=True).astype(int)

df_train.drop(columns=["input"], inplace=True)
df_test.drop(columns=["input"], inplace=True)

def feature_engineering(df):
    df["arg1%2"] = (df["arg1"]/16) % 2
    df["arg2%2"] = df["arg2"] % 2
    df["min(arg1 arg2)"] = df[["arg1", "arg2"]].min(axis=1)
    df["max(arg1 arg2)"] = df[["arg1", "arg2"]].max(axis=1)
    df["log(arg1*arg1)"] = np.log1p((df["arg1"]/16) * (df["arg1"]/16))
    df["log(arg1*arg1)/log(arg2)"] = np.log1p((df["arg1"]/16) * (df["arg1"]/16)) / np.log1p(df["arg2"])
    df["log(arg1*arg2)"] = np.log1p((df["arg1"]/16) * df["arg2"])
    df["arg1^2"] = np.log1p((df["arg1"]/16) ** 2)
    df["arg2^2"] = np.log1p(df["arg2"] ** 2)
    df["log(arg1)"] = np.log1p((df["arg1"]/16))
...
# ===== Group mapping =====
sdc_grouping_info = [[0.0 , 1.785], [1.785 , 2.57], [2.57 , 3.355],[3.355 , 4.14],[4.14 ,4.925],[4.925 , 5.71],[5.71 , 6.495],[6.495 , 7.28],[7.28 , 8.065],[8.065 , 8.85],[8.85 , 9.635],[9.635 , 10.42],[10.42 , 11.205],[11.205, 100.0]]
...
for input_str, predicted_sdc in result_array:
    ...
    os.system(f"cp -r ../../b-Offline-PreProtected-Binary-Generation/b-Needle/protected-binary-for-{pred_rep.replace(' ', '-')} protected-binary-for-{input_str.replace(' ', '-')}")
