...
df_train[["arg1", "arg2", "arg3", "arg4"]] = df_train["input"].str.replace('"', '').str.split(" ", expand=True)
df_test[["arg1", "arg2", "arg3", "arg4"]] = df_test["input"].str.replace('"', '').str.split(" ", expand=True)
df_train[["arg2", "arg3", "arg4"]] = df_train[["arg2", "arg3", "arg4"]].astype(int)
df_test[["arg2", "arg3", "arg4"]] = df_test[["arg2", "arg3", "arg4"]].astype(int)

df_train.drop(columns=["input", "arg1"], inplace=True)
df_test.drop(columns=["input", "arg1"], inplace=True)
...
def feature_engineering(df):
    ...
    df["log(arg3+arg4)"] = np.log1p(df["arg3"] + df["arg4"])
    df["log(arg2*10)"] = np.log1p(df["arg2"] * 10)
    df["log(arg3*arg4)/log(arg2)"] = np.log1p(df["arg3"] * df["arg4"]) / np.log1p(df["arg2"])
    return df
...
for input_str, predicted_sdc in result_array:
    ...
    os.system(f"cp -r ../../b-Offline-PreProtected-Binary-Generation/f-NN/protected-binary-for-{pred_rep.replace(' ', '-')} protected-binary-for-{input_str.replace(' ', '-')}")
