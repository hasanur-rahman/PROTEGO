...
df_train_split = df_train["input"].str.replace('"', '').str.split(" ", expand=True)
df_test_split = df_test["input"].str.replace('"', '').str.split(" ", expand=True)

# Extract numerical part from each relevant flag
df_train["arg1"] = df_train_split[0].str.extract(r"-m(\d+)")
df_train["arg2"] = df_train_split[1].str.extract(r"-p(\d+)")
df_train["arg3"] = df_train_split[2].str.extract(r"-n(\d+)")
df_train["arg4"] = df_train_split[3].str.extract(r"-l(\d+)")

df_test["arg1"] = df_test_split[0].str.extract(r"-m(\d+)")
df_test["arg2"] = df_test_split[1].str.extract(r"-p(\d+)")
df_test["arg3"] = df_test_split[2].str.extract(r"-n(\d+)")
df_test["arg4"] = df_test_split[3].str.extract(r"-l(\d+)")

# Convert extracted columns to integers
df_train[["arg1", "arg2", "arg3", "arg4"]] = df_train[["arg1", "arg2", "arg3", "arg4"]].astype(int)
df_test[["arg1", "arg2", "arg3", "arg4"]] = df_test[["arg1", "arg2", "arg3", "arg4"]].astype(int)

# Drop original input column
df_train.drop(columns=["input", "arg2"], inplace=True)
df_test.drop(columns=["input", "arg2"], inplace=True)
...
def feature_engineering(df):
    ...
    df["log(arg1*arg3*arg4)"] = np.log1p(df["arg1"] * df["arg3"] * df["arg4"])  
    df["log(arg1+arg3+arg4)"] = np.log1p(df["arg1"] + df["arg3"] + df["arg4"])
    df["log(arg1*arg4)/log(arg3)"] = np.log1p(df["arg1"] * df["arg4"]) / np.log1p(df["arg3"])
    df["min(arg1 arg3 arg4)"] = df[["arg1", "arg3", "arg4"]].min(axis=1)
    df["max(arg1 arg3 arg4)"] = df[["arg1", "arg3", "arg4"]].max(axis=1)
    df["arg1^2"] = np.log1p(df["arg1"] ** 2)
    df["arg3^2"] = np.log1p(df["arg3"] ** 2)
    df["arg4^2"] = np.log1p(df["arg4"] ** 2)
    df["log(arg1)"] = np.log1p(df["arg1"])
    df["log(arg3)"] = np.log1p(df["arg3"])
...
for input_str, predicted_sdc in result_array:
    ...
    os.system(f"cp -r ../../b-Offline-PreProtected-Binary-Generation/h-FFT/protected-binary-for-{pred_rep.replace(' ', '-')} protected-binary-for-{input_str.replace(' ', '-')}")
