...
def feature_engineering(df):
    ...
    df["arg2%2"] = df["arg2"] % 2
    df["arg3%2"] = df["arg3"] % 2
    df["min(arg1 arg2 arg3)"] = df[["arg1", "arg2", "arg3"]].min(axis=1)
    df["max(arg1 arg2 arg3)"] = df[["arg1", "arg2", "arg3"]].max(axis=1)
    df["arg1*arg2*arg3"] = df["arg1"] * df["arg2"]* df["arg2"]
    df["log(arg1+arg2+arg3)"] = np.log1p(df["arg1"] + df["arg2"]+ df["arg3"])
    df["log(arg1*arg2*arg3)"] = np.log1p(df["arg1"] * df["arg2"] * df["arg3"])  
    return df
...
for input_str, predicted_sdc in result_array:
    ...
    os.system(f"cp -r ../../b-Offline-PreProtected-Binary-Generation/i-HPCCG/protected-binary-for-{pred_rep.replace(' ', '-')} protected-binary-for-{input_str.replace(' ', '-')}")
